...
- [ ] markdown-it によるGFM変換（テーブル・タスクリスト・打ち消し線・自動リンク）
- [ ] 見出しへのアンカーID付与と `headings`（ページ内目次用データ）の抽出
- [ ] 内部リンクの書き換え（`.md` → `.html`、アンカー付きリンク対応）
- [ ] 検索用プレーンテキスト抽出（HTMLタグ除去）
- [ ] `markdown.allow_html: false` 時の生HTML無効化
...
- [ ] 内部リンク検証（相対リンク・アンカーの存在チェック、切れリンクの一覧警告）
- [ ] `--strict` オプション（警告をエラー化し終了コード1）
- [ ] `--clean` オプション（出力ディレクトリの事前クリア）
- [ ] `init` コマンド（`mkdocsgen.yml` / `docs/index.md` / `docs/guide/getting-started.md` の雛形生成、既存ファイルはスキップ + 警告）
- [ ] ビルドサマリ表示（ページ数・警告数・所要時間）
...
- [ ] 検索インデックス生成（タイトル・見出し・本文テキスト、bigramトークン化、`search-index.json` 出力）
- [ ] MiniSearchによるクライアントサイド検索（初回フォーカス時の遅延ロード）
- [ ] 検索UI（インクリメンタル検索、ドロップダウン結果表示、0件メッセージ、Enter/クリック/Escの操作、`/` キーショートカット）
...
- [ ] HTTPサーバー（localhostバインドのみ、`--port` 対応）
- [ ] chokidarによる監視（`docs/` / `mkdocsgen.yml` / テーマオーバーライド / pydoc対象ソース）
- [ ] WebSocketによるライブリロード
- [ ] 増分再ビルド（変更ページのみ再変換、nav影響時の範囲判定は仕様書8.3の懸念に留意）
- [ ] 設定ファイル変更時のフルビルド
- [ ] ビルドエラー時のエラーオーバーレイ表示と自動復帰
